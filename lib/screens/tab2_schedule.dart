...
class _Tab2ScheduleState extends State<Tab2Schedule> {
  ...
  // 선택 상태는 ScheduleBloc, 현재 시각은 ClockBloc 에서 읽는다.
  int get _selectedDockIndex =>
      context.read<ScheduleBloc>().state.selectedDockIndex;
  ScheduleDirection get _selectedDirection =>
      context.read<ScheduleBloc>().state.selectedDirection;
  ...
  int _currentMinutes() => context.read<ClockBloc>().state.minuteOfDay;

  @override
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context)!;
    // 선택 상태 변화와 분 단위 시계 변화에 반응한다.
  ...
  Widget _buildRouteStop(
    ...
    return GestureDetector(
      onTap: () => context.read<ScheduleBloc>().add(
        ScheduleDockSelected(_docks.indexOf(dock)),
      ),
      child: Column(
