...
class Tab1Home extends StatefulWidget {
  ...
  const Tab1Home({super.key, this.onNavigateTab});
...
class _Tab1HomeState extends State<Tab1Home> with TickerProviderStateMixin {
  ...
  final Map<String, HangangRealtimeData> _realtimeData = {};
  ...
  void initState() {
    ...
    _realtimeDataTimer = Timer.periodic(
  ...
  Future<void> _fetchWeather() async {
    ...
    const primaryPark = '마곡나루역';
    const parks = [primaryPark, '망원한강공원', '여의도한강공원', '뚝섬한강공원', '잠실한강공원'];

    final quickWeather = await WeatherService.fetchSeoulFallback();
    if (!mounted) return;
    if (quickWeather != null) {
      setState(() {
        _weatherData = {
          ..._weatherData,
          primaryPark: quickWeather,
          '여의도한강공원': quickWeather,
        };
        _weatherLoading = false;
      });
    }

    final primaryWeather = await WeatherService.fetch(primaryPark);
    if (!mounted) return;
    if (primaryWeather != null) {
      setState(() {
        _weatherData = {
          ..._weatherData,
          primaryPark: primaryWeather,
          '여의도한강공원': primaryWeather,
        };
        _weatherLoading = false;
      });
    }

    for (final park in parks.where((p) => p != primaryPark)) {
      final weather = await WeatherService.fetch(park);
      if (!mounted) return;
      if (weather != null) {
        setState(() {
          _weatherData = {..._weatherData, park: weather};
        });
      }
    }
  ...
  WeatherData? _getHeaderWeather(DockInfo dock) {
    final parkAreaName = dock.parkAreaName;
    if (parkAreaName != null && _weatherData.containsKey(parkAreaName)) {
      return _weatherData[parkAreaName];
    }
    return _weatherData.isNotEmpty ? _weatherData.values.first : null;
  }

  Future<void> _fetchRealtimeData() async {
    // HangangRealtimeService.fetchAll() 호출 위치
  }
  ...
  Widget build(BuildContext context) {
    ...
    final headerWeather = _getHeaderWeather(currentDock);
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          _buildStat(
                            l10n.statTrips,
                            l10n.statTripsValue(18), // 👈 '18회' 또는 '18'로 자동 변환
                            Icons.sailing,
                            isDarkMode,
                          ),
                          const SizedBox(width: 12),
