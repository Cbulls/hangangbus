import 'package:hangangbus/models/data.dart';

final List<StoryItem> storyDataKo = [
  // ========== 여의도 ==========
  StoryItem(
    category: 'HISTORY',
    title: '여의도 윤중제',
    subtitle: '한강의 범람을 막아낸 제방의 역사',
    imageUrl: 'assets/images/yeoui_yunje.jpeg',
    description:
        '여의도를 둘러싼 윤중제는 1968년 한강개발사업의 일환으로 축조되었습니다. '
        '이전까지 여의도는 홍수 때마다 침수되는 모래섬이었으나, '
        '윤중제 덕분에 현재의 정치·금융 중심지로 발전할 수 있었습니다.\n\n'
        '제방을 따라 조성된 산책로는 시민들의 휴식 공간이자 벚꽃 명소로 사랑받고 있습니다.',
    dockName: '여의도',
    accessInfo: '선착장에서 도보 5분',
    historicalPeriod: '1968년 축조',
  ),
  StoryItem(
    category: 'HISTORY',
    title: '63빌딩과 여의도 개발',
    subtitle: '대한민국 랜드마크의 탄생',
    imageUrl: 'assets/images/63building.jpg',
    description:
        '1985년 완공된 63빌딩은 당시 아시아에서 가장 높은 건물이었습니다. '
        '황금빛 외관은 한강의 물결을 형상화한 것으로, '
        '대한민국의 경제 성장을 상징하는 건축물입니다.\n\n'
        '여의도는 1970년대부터 국회의사당, 방송국, 금융기관이 들어서며 '
        '대한민국의 정치·경제·문화의 중심지로 자리잡았습니다.',
    dockName: '여의도',
    accessInfo: '선착장에서 도보 22분',
    historicalPeriod: '1985년',
  ),
  StoryItem(
    category: 'HISTORY',
    title: '여의도 공원의 탄생',
    subtitle: '시민 광장으로의 변천사',
    imageUrl: 'assets/images/yeoui_plaza.jpg',
    description:
        '현재의 여의도공원은 원래 5.16 광장이라는 이름의 아스팔트 광장이었습니다. '
        '군사 퍼레이드와 대규모 집회를 위해 조성되었으나, '
        '1999년 시민의 휴식 공간인 공원으로 재탄생했습니다.\n\n'
        '잔디광장, 한국 전통의 숲, 자연생태의 숲으로 구성되어 '
        '도심 속 자연을 만끽할 수 있는 명소가 되었습니다.',
    dockName: '여의도',
    accessInfo: '선착장에서 도보 15분',
    historicalPeriod: '1999년 공원 조성',
  ),
  StoryItem(
    category: 'FOOD',
    title: '호시우보 여의도TP타워점',
    subtitle: '김호윤 셰프의 프리미엄 한우 다이닝',
    imageUrl: 'assets/images/hosiubo.jpeg',
    description:
        '흑백요리사 김호윤 오너셰프의 한우 전문점으로, 최상급 한우를 현대적으로 재해석한 코스 요리와 구이를 선보입니다. '
        '여의도의 새로운 랜드마크인 TP타워에 위치하여 세련된 인테리어와 고급스러운 분위기를 자랑합니다.\n\n'
        '정갈한 찬과 섬세한 서비스로 비즈니스 미팅이나 특별한 기념일 식사에 최적화된 공간입니다.',
    dockName: '여의도',
    accessInfo: '선착장에서 도보 10분 (TP타워 2층)',
    openingHours: '11:00 - 22:00 (브레이크타임 15:00 - 17:00)',
    priceRange: '50,000원 - 150,000원',
  ),
  StoryItem(
    category: 'FOOD',
    title: '진주집',
    subtitle: '여의도를 대표하는 콩국수 노포 맛집',
    imageUrl: 'assets/images/jinjujeob.jpeg',
    description:
        '전국적인 인지도를 자랑하는 여의도 최고의 노포로, 설탕이나 소금 없이도 고소한 풍미가 일품인 걸쭉한 냉콩국수가 시그니처입니다. '
        '닭칼국수와 비빔국수, 그리고 속이 꽉 찬 접시만두 또한 빼놓을 수 없는 별미입니다.\n\n'
        '사계절 내내 줄을 서는 맛집이지만 빠른 회전율을 자랑하며, 여의도 직장인들의 소울푸드 점심 장소로 꼽힙니다.',
    dockName: '여의도',
    accessInfo: '선착장에서 도보 15분 (여의도백화점 지하 1층)',
    openingHours: '10:00 - 20:00 (일요일 휴무)',
    priceRange: '12,000원 - 15,00:0원',
  ),
  StoryItem(
    category: 'FOOD',
    title: '이도 여의도',
    subtitle: '정갈한 한식 다이닝과 곰탕의 조화',
    imageUrl: 'assets/images/yido.png',
    description:
        '깊고 맑은 국물 맛이 일품인 곰탕과 수육, 그리고 정갈한 한식 요리를 제공하는 프리미엄 한식당입니다. '
        '화학 조미료를 최소화하고 식재료 본연의 맛을 살린 건강한 한 끼를 제공하여 중장년층과 젊은 층 모두에게 사랑받습니다.\n\n'
        '모던한 공간 디자인 덕분에 여의도 한복판에서 차분하고 품격 있는 식사를 즐기기에 좋습니다.',
    dockName: '여의도',
    accessInfo: '선착장에서 도보 12분 (여의도 파이낸스타워 인근)',
    openingHours: '11:00 - 21:30 (브레이크타임 15:00 - 17:00)',
    priceRange: '15,000원 - 45,000원',
  ),

  // ========== 망원 ==========
  StoryItem(
    category: 'HISTORY',
    title: '망원정과 조선시대 유람',
    subtitle: '한강을 바라보던 정자의 흔적',
    imageUrl: 'assets/images/mangwonjeong.jpeg',
    description:
        '망원(望遠)이란 이름은 조선시대 이곳에 있던 망원정(望遠亭)에서 유래했습니다. '
        '망원정은 한강의 경치를 조망하기 위해 세워진 정자로, '
        '선비들이 시를 짓고 풍류를 즐기던 명소였습니다.\n\n'
        '현재 망원정은 남아있지 않지만, 망원한강공원에 그 이름이 계승되어 '
        '여전히 많은 시민들이 한강의 아름다운 풍경을 감상하고 있습니다.',
    dockName: '망원',
    accessInfo: '선착장에서 도보 3분',
    historicalPeriod: '조선시대',
  ),
  StoryItem(
    category: 'HISTORY',
    title: '양화진 외국인 선교사 묘원',
    subtitle: '한국 근대사의 산 교육장',
    imageUrl: 'assets/images/yanghwajin.jpg',
    description:
        '양화진은 19세기 후반부터 한국에 온 외국인 선교사들의 묘지로 사용되었습니다. '
        '언더우드, 아펜젤러 등 한국 근대 교육과 의료에 헌신한 선교사들이 잠들어 있으며, '
        '현재는 역사 교육의 장으로 활용되고 있습니다.\n\n'
        '양화진외국인선교사묘원역사관에서 그들의 삶과 업적을 배울 수 있습니다.',
    dockName: '망원',
    accessInfo: '선착장에서 마을버스 10분',
    historicalPeriod: '1890년대~',
  ),
  StoryItem(
    category: 'HISTORY',
    title: '망원시장의 변천',
    subtitle: '전통시장에서 핫플레이스로',
    imageUrl: 'assets/images/mangwon_market.jpeg',
    description:
        '1970년대 형성된 망원시장은 마포구 주민들의 생활 중심지였습니다. '
        '2000년대 들어 젊은 예술가들이 모여들며 독특한 카페와 가게들이 생겨났고, '
        '현재는 전통과 현대가 공존하는 문화 공간으로 재탄생했습니다.\n\n'
        '신선한 재료를 파는 전통 상점과 감각적인 카페가 어우러진 '
        '망원시장만의 독특한 분위기를 느낄 수 있습니다.',
    dockName: '망원',
    accessInfo: '선착장에서 도보 12분',
    historicalPeriod: '1970년대 형성',
  ),
  StoryItem(
    category: 'FOOD',
    title: '옥동식',
    subtitle: '미쉐린 가이드가 인정한 맑은 돼지곰탕',
    imageUrl: 'assets/images/okdongsik.jpeg',
    description:
        '흑백요리사 옥동식 셰프의 본점으로, 기존의 탁한 돼지국밥과 달리 맑고 투명한 국물이 특징인 돼지곰탕을 선보입니다. '
        '버크셔K 흑돼지만을 사용하여 고기 자체의 감칠맛이 뛰어나며, 유기그릇에 담겨 나오는 정갈함이 돋보입니다.\n\n'
        '미쉐린 가이드 빕 구르망에 꾸준히 선정되는 곳으로, 바(Bar) 좌석으로 구성되어 혼밥 하기에도 좋습니다.',
    dockName: '망원',
    accessInfo: '서울특별시 마포구 양화로7길 44-10',
    openingHours: '11:00 - 22:00 (브레이크타임 15:00 - 17:00)',
    priceRange: '10,000원 - 16,000원',
  ),
  StoryItem(
    category: 'FOOD',
    title: '고향집',
    subtitle: '망원시장의 전설, 가성비 최고의 칼국수',
    imageUrl: 'assets/images/gohyangjib.jpeg',
    description:
        '망원시장을 대표하는 노포 맛집으로, 저렴한 가격에도 불구하고 깊은 멸치 육수 맛을 자랑하는 칼국수와 수제비가 일품입니다. '
        '직접 반죽한 면발의 쫄깃함이 특징이며, 함께 나오는 겉절이 김치와의 조화가 훌륭합니다.\n\n'
        '시장의 활기찬 분위기 속에서 든든하게 한 끼를 해결할 수 있는 망원동의 필수 코스입니다.',
    dockName: '망원',
    accessInfo: '서울특별시 마포구 포은로8길 28',
    openingHours: '11:00 - 21:00',
    priceRange: '5,000원 - 8,000원',
  ),
  StoryItem(
    category: 'FOOD',
    title: '오스테리아샘킴',
    subtitle: '샘킴 셰프의 따뜻한 이탈리안 오픈 키친',
    imageUrl: 'assets/images/osteriasamkim.jpeg',
    description:
        '냉장고를 부탁해로 잘 알려진 샘킴 셰프가 운영하는 이탈리안 레스토랑입니다. '
        '오픈 키친을 통해 요리하는 모습을 직접 볼 수 있으며, 신선한 제철 식재료를 사용한 파스타와 스테이크를 제공합니다.\n\n'
        '망원동 특유의 아늑한 분위기 속에서 품격 있는 요리를 즐길 수 있어 연인들의 데이트 코스로 인기가 높습니다.',
    dockName: '망원',
    accessInfo: '서울특별시 마포구 희우정로20길 71',
    openingHours: '11:30 - 22:00 (브레이크타임 14:30 - 17:30 / 일요일 휴무)',
    priceRange: '25,000원 - 60,000원',
  ),

  // ========== 마곡 ==========
  StoryItem(
    category: 'HISTORY',
    title: '겸재 정선과 미술관',
    subtitle: '진경산수화의 성인이 머문 마곡의 역사',
    imageUrl: 'assets/images/gyeomjae_museum.jpeg',
    description:
        '조선 후기 진경산수화의 대가인 겸재 정선(1676-1759)은 65세의 나이에 현재의 강서구 가양동 일대인 "양천현아"의 현감으로 부임했습니다. '
        '그는 이곳에 머무는 동안 한강의 아름다운 풍광에 매료되어 "경교명승첩" 등 수많은 한강 관련 걸작들을 남겼습니다.\n\n'
        '겸재정선미술관은 그의 예술적 혼이 깃든 터에 세워졌으며, 조선 역사상 가장 뛰어난 화가 중 한 명인 그의 작품을 통해 300년 전 한강의 원형을 마주할 수 있습니다.',
    dockName: '마곡',
    accessInfo: '서울특별시 강서구 양천로47길 36',
    historicalPeriod: '조선 후기 (18세기)',
  ),
  StoryItem(
    category: 'HISTORY',
    title: '김포공항과 항공 산업',
    subtitle: '대한민국 하늘길의 시작',
    imageUrl: 'assets/images/gimpo_airport.jpg',
    description:
        '1958년 개항한 김포공항은 대한민국 항공 산업의 출발점입니다. '
        '한국전쟁 중 미군이 사용하던 활주로를 민간 공항으로 전환하며 시작되었고, '
        '2001년 인천공항 개항 전까지 대한민국의 관문 역할을 했습니다.\n\n'
        '현재는 국내선과 단거리 국제선을 운영하며, '
        '인근 마곡지구는 첨단 산업 단지로 발전했습니다.',
    dockName: '마곡',
    accessInfo: '선착장에서 버스 20분',
    historicalPeriod: '1958년 개항',
  ),
  StoryItem(
    category: 'HISTORY',
    title: '마곡지구 개발',
    subtitle: '서울의 신(新) R&D 허브',
    imageUrl: 'assets/images/magok_district.jpg',
    description:
        '2000년대 후반부터 개발이 시작된 마곡지구는 '
        '서울시의 미래 성장동력으로 기획된 첨단 산업 단지입니다.\n\n'
        'LG사이언스파크를 비롯한 R&D 기업들이 입주해 있으며, '
        '친환경 스마트시티를 목표로 조성되어 녹지 공간과 문화 시설이 풍부합니다.',
    dockName: '마곡',
    accessInfo: '선착장에서 도보 10분',
    historicalPeriod: '2000년대 후반~',
  ),
  StoryItem(
    category: 'FOOD',
    title: '롤리밥스',
    subtitle: '마곡나루 직장인들이 애정하는 프리미엄 분식',
    imageUrl: 'assets/images/lollybobs.jpeg',
    description:
        '다양한 종류의 김밥과 떡볶이, 돈가스 등을 제공하는 마곡나루역 인근의 인기 분식점입니다. '
        '신선한 재료를 듬뿍 넣어 정성스럽게 말아낸 김밥은 바쁜 직장인들의 든든한 점심 식사로 인기가 높습니다.\n\n'
        '합리적인 가격과 깔끔한 맛 덕분에 한강 산책 전후로 가볍게 포장하거나 식사하기에 최적의 장소입니다.',
    dockName: '마곡',
    accessInfo: '서울특별시 강서구 마곡중앙5로 6',
    openingHours: '08:00 - 21:00 (토요일은 20:00까지 / 일요일 휴무)',
    priceRange: '5,000원 - 12,000원',
  ),
  StoryItem(
    category: 'FOOD',
    title: '은하식당',
    subtitle: '정성 가득한 집밥 스타일의 한식 전문점',
    imageUrl: 'assets/images/eunhasikdang.jpeg',
    description:
        'LG아트센터 서울과 서울식물원 인근에 위치하여 깔끔하고 정갈한 한식 반상을 선보이는 곳입니다. '
        '자극적이지 않고 건강한 맛의 밑반찬과 메인 요리가 어우러져 부모님이나 아이와 함께 방문하기 좋습니다.\n\n'
        '모던하고 따뜻한 인테리어 덕분에 차분한 분위기에서 식사를 즐길 수 있으며, 식물원 나들이 후 방문을 추천합니다.',
    dockName: '마곡',
    accessInfo: '서울특별시 강서구 마곡동로 161',
    openingHours: '11:00 - 21:30 (브레이크타임 15:30 - 17:00)',
    priceRange: '10,000원 - 20,000원',
  ),
  StoryItem(
    category: 'FOOD',
    title: '일오삼김밥',
    subtitle: '엄마의 마음으로 만든 정직한 김밥',
    imageUrl: 'assets/images/153gimbap.jpeg',
    description:
        '화학 조미료를 최소화하고 좋은 재료를 고집하는 마곡 지구의 숨은 김밥 맛집입니다. '
        '기본에 충실한 153김밥부터 속이 꽉 찬 참치, 치즈 김밥까지 집에서 만든 듯한 담백한 맛이 일품입니다.\n\n'
        '간단하지만 영양가 있는 식사를 원하는 분들에게 추천하며, 서울식물원 피크닉을 위한 도시락으로도 인기가 많습니다.',
    dockName: '마곡',
    accessInfo: '서울특별시 강서구 마곡중앙로 161-11',
    openingHours: '07:30 - 20:00 (재료 소진 시 마감 / 일요일 휴무)',
    priceRange: '4,000원 - 8,000원',
  ),
  // ========== 옥수 ==========
  StoryItem(
    category: 'HISTORY',
    title: '두모포와 조선의 얼',
    subtitle: '조선 시대 군사 요충지이자 문인들의 쉼터',
    imageUrl: 'assets/images/dumopo.jpg',
    description:
        '옥수동의 옛 이름인 \'두모포(豆毛浦)\'는 중랑천과 한강이 만나는 지점으로, 조선 시대 동해안과 강원도 지방으로 향하는 중요한 나루터이자 군사 요충지였습니다.\n\n'
        '조선 세종 시절 대마도 정벌을 위해 이종무 장군이 이끄는 군대가 출정식을 가졌던 역사적인 장소이기도 하며, 빼어난 경치 덕분에 선비들이 시를 짓고 풍류를 즐기던 곳입니다.',
    dockName: '옥수',
    accessInfo: '선착장에서 도보 5분',
    historicalPeriod: '조선시대',
  ),
  StoryItem(
    category: 'HISTORY',
    title: '독서당 신성터',
    subtitle: '조선 엘리트 학자들의 독서 휴가제',
    imageUrl: 'assets/images/dokseodang.jpeg',
    description:
        '조선 성종 시기, 젊고 유능한 학자들에게 휴가를 주어 학문에 전념하게 하던 \'사독서(賜讀書)\' 제도를 위해 옥수동 인근에 동호독서당(東湖讀書堂)이 세워졌습니다.\n\n'
        '이황, 이이 등 당대 최고의 석학들이 이곳에서 한강을 바라보며 학문을 닦았으며, 오늘날 옥수동 일대의 \'독서당로\'라는 도로명에 그 역사의 흔적이 고스란히 남아있습니다.',
    dockName: '옥수',
    accessInfo: '선착장에서 도보 15분',
    historicalPeriod: '조선 중기',
  ),
  StoryItem(
    category: 'FOOD',
    title: '고향냉면',
    subtitle: '금호역 주변 금남시장 숨은 맛집이자 착한 가격 업소',
    imageUrl: 'assets/images/gohyang.jpeg',
    description:
        '금호역 주변 금남시장 골목 안쪽에 위치한 고향냉면은 정겨운 전통시장의 정취를 그대로 간직한 숨은 로컬 맛집입니다. '
        '정직하고 저렴한 가격을 고수하여 정부와 지자체가 지정한 \'착한 가격 업소\'로 선정된 곳입니다.\n\n'
        '이곳의 대표 메뉴인 새콤달콤한 냉면(물/비빔)을 비롯해 열무와 콩나물이 듬뿍 올라간 보리비빔밥, 구수한 된장찌개 등 정갈한 한식을 선보입니다. '
        '특히 찬 바람이 불 때 손님들이 즐겨 찾는 푸짐한 칼만두국은 진한 국물과 쫄깃한 면발이 어우러져 따끈한 칼만두국이 전하는 겨울의 행복을 그대로 느낄 수 있습니다.',
    dockName: '옥수',
    accessInfo: '서울 성동구 독서당로 301-93 고향냉면',
    priceRange: '8,000원 - 9,000원',
    openingHours: '11:00 - 20:00 (브레이크타임 15:00 - 16:00 / 일요일 휴무)',
  ),
  StoryItem(
    category: 'FOOD',
    title: '옥수해물찜칼국수',
    subtitle: '그릇을 넘쳐흐르는 압도적이고 푸짐한 양의 맛집',
    imageUrl: 'assets/images/oksu_haemuljjim.jpeg',
    description:
        '옥수역 4번 출구 인근에 위치한 옥수해물찜칼국수는 해물찜, 아구찜, 바지락 칼국수 등 다양한 해산물 요리를 맛볼 수 있는 식당입니다.\n\n'
        '이곳의 해물찜은 맵지 않고 심심한 맛에 가깝고 낙지의 쫀득한 식감이 특징이며, 아구찜은 살이 튼실하고 신선한 해산물이 풍부하게 들어있습니다. '
        '특히 양이 매우 푸짐하여 4인 가족이 방문해도 해물찜 소(小) 자를 시키면 충분히 먹을 수 있을 정도로 넉넉한 인심을 자랑하는 곳입니다.',
    dockName: '옥수',
    accessInfo: '서울 성동구 독서당로40길 39 옥수 어울림',
    openingHours: '11:30 - 21:30 (연중무휴)',
    priceRange: '10,000원 - 69,000원',
  ),

  // ========== 서울숲 ==========
  StoryItem(
    category: 'HISTORY',
    title: '서울숲의 역사',
    subtitle: '왕의 사냥터에서 탄생한 도심 속 생명의 숲',
    imageUrl: 'assets/images/seoul_forest_history.jpg',
    description:
        '서울숲은 과거 임금의 사냥터였으며, 1908년 설치된 서울 최초의 상수원 수원지이기도 했습니다. 이후 경마장과 골프장 등으로 활용되다가, 2002년 시민의 녹색 권리를 위해 개발 대신 공원을 조성하기로 결정되었습니다.\n\n'
        '뉴욕 센트럴 파크에 버금가는 웰빙 공간을 목표로, 5,000여 명 시민의 기금과 봉사를 통해 2005년 6월 18일 울창한 나무와 호수가 있는 도시숲으로 탈바꿈했습니다. '
        '현재는 한강과 중랑천을 잇는 서울의 핵심 녹지 축으로서 자연생태숲, 습지생태원 등 4개의 테마공원을 갖춘 서울의 대표적인 쉼터 역할을 하고 있습니다.',
    dockName: '서울숲',
    accessInfo: '수인분당선 서울숲역 4번 출구 도보 2분 (선착장 도보 5분)',
    historicalPeriod: '조선시대 ~ 현재',
  ),
  StoryItem(
    category: 'HISTORY',
    title: '수도박물관과 뚝도수원지',
    subtitle: '대한민국 근대 상수도의 역사적 시작점',
    imageUrl: 'assets/images/waterworks_museum.jpeg',
    description:
        '수도박물관이 서울숲(뚝섬)에 위치한 이유는 이곳이 1908년 대한제국 시절 완공된 한국 최초의 근대식 정수장인 \'뚝도수원지 제1정수장\'이 있던 역사적 터전이기 때문입니다. 당시 뚝섬 일대는 중랑천과 한강이 만나며 모래 여과층이 자연적으로 형성되어, 한강의 깨끗한 물을 취수하기에 가장 이상적인 지형적 조건을 갖추고 있었습니다.\n\n'
        '역사적으로 이 시설의 완공은 오랫동안 우물물과 강물에 의존하며 콜레라 등 수인성 전염병에 취약했던 서울 시민들에게 위생적이고 안전한 물을 공급하게 된 획기적인 보건 혁명의 시작이었습니다. 100년이 넘는 세월 동안 서울의 폭발적인 성장과 시민의 생명을 지탱해 온 한국 근대화 및 도시 인프라 발전의 가장 상징적인 유적지입니다.',
    dockName: '서울숲',
    accessInfo: '선착장에서 도보 3분 (수도박물관 내)',
    historicalPeriod: '1908년 (대한제국) 건립',
  ),
  StoryItem(
    category: 'HISTORY',
    title: '성수동 수제화거리와 장인정신',
    subtitle: '대한민국 토종 제조업의 역사와 문화적 자산',
    imageUrl: 'assets/images/seongsu_shoes_street.webp',
    description:
        '성수동 수제화거리는 1970~80년대 명동과 회현동 일대의 수제화 업체들이 치솟는 임대료를 피해 넓은 부지와 원자재(가죽, 부속품) 조달이 용이했던 성수동 공장 지대로 이주하면서 형성되었습니다. 이곳은 한때 대한민국 신발 생산의 핵심 기지로서 한국 근대 제조업 부흥기를 이끈 산증인이자, 평생을 바쳐 기술을 갈고닦은 명장들의 숨결이 서린 공간입니다.\n\n'
        '이 거리를 기념하는 것은 대량 생산과 패스트 패션 시대 속에서도 사라져가는 전통 \'장인정신(Craftsmanship)\'의 독자적인 가치를 증명하고 계승하기 위함입니다. 붉은 벽돌의 옛 공장 지대가 오늘날 가장 트렌디한 문화예술 중심지로 변모하는 과정에서도, 성수동이 단순한 유행 소비지를 넘어 깊이 있는 정체성과 복합적인 역사적 서사를 품을 수 있게 해주는 뿌리 깊은 문화 자산입니다.',
    dockName: '서울숲',
    accessInfo: '지하철 성수역 일대 (선착장에서 도보 20분 / 따릉이 7분)',
    historicalPeriod: '1970년대 형성 ~ 현재',
  ),
  StoryItem(
    category: 'FOOD',
    title: '포레스트 서울숲',
    subtitle: '농장 직송 최상급 생딸기 디저트 천국',
    imageUrl: 'assets/images/forest_seoulsup.jpeg',
    description:
        '포레스트 서울숲은 매일 아침 농장에서 들여오는 최상급 생딸기를 활용한 디저트가 특징인 카페입니다.\n\n'
        '특히 생딸기 모찌 하겐다즈 파르페는 방문객들이 서울숲 디저트 카페로 손꼽는 대표적인 인기 메뉴입니다. '
        '1층부터 4층까지의 넓은 내부 공간과 감성적인 루프탑까지 마련되어 있어 데이트나 모임 장소로 적합하며, 애견 동반도 가능하여 부담 없이 방문하기 좋습니다.',
    dockName: '서울숲',
    accessInfo: '서울특별시 성동구 서울숲2길 20-12 1층~4층',
    openingHours: '10:00 - 21:00',
    priceRange: '8,000원 - 35,000원',
  ),
  StoryItem(
    category: 'FOOD',
    title: '할머니의 레시피',
    subtitle: '성수동 골목에서 만나는 따뜻한 가정식 한 상',
    imageUrl: 'assets/images/grandma_recipe.jpeg',
    description:
        '서울숲 인근에서 가장 유명한 한식당 중 하나로, 자극적이지 않고 담백한 집밥 스타일의 반상을 선보입니다. 쌈밥정식, 비빔밥불고기정식, 한우떡갈비 등이 대표 메뉴입니다.\n\n'
        '모던하고 깔끔한 단독주택 건물에서 통창을 통해 정원을 바라보며 정갈한 식사를 즐길 수 있어 외국인 관광객들도 많이 방문합니다.',
    dockName: '서울숲',
    accessInfo: '서울특별시 성동구 서울숲2길 44-12',
    openingHours: '11:00 - 21:00 (브레이크타임 15:30 - 16:40)',
    priceRange: '11,000원 - 25,000원',
  ),
  StoryItem(
    category: 'FOOD',
    title: '서울숲누룽지통닭구이',
    subtitle: '참나무 장작 향을 품은 담백한 통닭과 고소한 누룽지',
    imageUrl: 'assets/images/seoulsup_nurungji.jpeg',
    description:
        '서울숲누룽지통닭구이는 성수동 일대에서 오랜 시간 주민들과 직장인들에게 사랑받아 온 로컬 치맥 명소입니다. '
        '매장 외부에서 참나무 장작불로 오랜 시간 서서히 구워내어, 기름기는 쏙 빠지고 닭고기 특유의 담백함과 촉촉한 육즙이 그대로 살아있는 것이 특징입니다.\n\n'
        '뜨거운 돌판 바닥에 찰밥이 눌어붙어 만들어지는 고소하고 바삭한 누룽지가 이 집의 핵심 별미이며, '
        '기본 누룽지통닭 외에도 치즈와 옥수수가 가득 올라간 \'누룽지 콘치즈\', 알싸한 파를 곁들인 \'누룽지 파닭\'이 대표 메뉴입니다. '
        '함께 파는 국물떡볶이를 곁들여 시원한 생맥주와 즐기기 좋으며, 서울숲 나들이나 한강 산책 후 하루를 마무리하기에 제격입니다.',
    dockName: '서울숲',
    accessInfo: '서울특별시 성동구 왕십리로5길 10',
    openingHours: '월~금 16:00 - 23:00 / 토~일 14:00 - 23:00',
    priceRange: '24,000원 - 30,000원',
  ),

  // ========== 뚝섬 ==========
  StoryItem(
    category: 'HISTORY',
    title: '뚝섬의 유래',
    subtitle: '섬이 아닌 뚝섬에 깃든 왕의 깃발 이야기',
    imageUrl: 'assets/images/ttukseom_origin.jpeg',
    description:
        '뚝섬은 이름에 \'섬\'이 들어가지만 실제로는 섬이 아닙니다. 한강과 중랑천이 만나는 지점에 발달한 범람원 지형으로, 평소에는 육지였다가 큰 비가 내려 물이 범람하면 마치 섬처럼 고립되던 것에서 유래했습니다.\n\n'
        '\'뚝섬\'이라는 지명은 왕의 수레 앞에 세우던 상징적인 깃발인 \'둑기\'에서 시작되었습니다. 조선 시대 왕들은 이곳에 나와 군대를 사열하거나 사냥을 즐겼고, 매년 둑기를 세우고 \'둑제\'라는 제사를 지냈습니다. 이 때문에 \'둑기를 꽂은 섬 같은 곳\'이라는 뜻으로 \'둑도\'라 불리던 것이 음이 변하여 오늘날의 \'뚝섬\'이 되었습니다.',
    dockName: '뚝섬',
    accessInfo: '선착장 주변 수변 일대',
    historicalPeriod: '조선시대',
  ),
  StoryItem(
    category: 'HISTORY',
    title: '뚝섬유원지의 시작',
    subtitle: '서민들의 오랜 강변 휴양지',
    imageUrl: 'assets/images/ttukseom_resort.jpeg',
    description:
        '뚝섬은 1930년대부터 뚝섬유원지로 개발되어 서울 시민들이 여름철 물놀이와 뱃놀이를 즐기던 유서 깊은 휴양 공간이었습니다.\n\n'
        '1980년대 한강 종합개발사업을 거쳐 현재의 현대적인 한강공원의 틀을 갖추었으며, 윈드서핑 등 수상 레포츠의 메카로서 그 활기찬 명성을 이어가고 있습니다.',
    dockName: '뚝섬',
    accessInfo: '선착장에서 도보 1분',
    historicalPeriod: '1930년대~현재',
  ),
  StoryItem(
    category: 'FOOD',
    title: '자양전통시장',
    subtitle: '뚝섬 자양동 주민들의 활력 넘치는 먹거리 천국',
    imageUrl: 'assets/images/jayang_market.jpeg',
    description:
        '오랜 역사를 자랑하는 자양동의 대표 전통시장으로, 줄 서서 먹는 마늘 떡볶이와 닭강정, 족발 등 가성비 훌륭한 로컬 먹거리가 가득합니다.\n\n'
        '온누리상품권 사용이 가능하고 시장 내부가 깔끔하게 정비되어 있어, 뚝섬 한강공원 피크닉을 가기 전 주전부리를 포장하기에 최적의 장소입니다.',
    dockName: '뚝섬',
    accessInfo: '서울특별시 광진구 자양로13길 64',
    openingHours: '09:00 - 22:00 (점포별 상이)',
    priceRange: '점포별 상이',
  ),
  StoryItem(
    category: 'FOOD',
    title: '청도양꼬치',
    subtitle: '잡내 없이 육즙 가득한 건대 양꼬치 거리의 숨은 명소',
    imageUrl: 'assets/images/tsingtao_lamb.jpeg',
    description:
        '뚝섬 선착장 인근 자양동 건대 양꼬치 거리에 위치한 청도양꼬치는 엄선된 1년 미만의 양고기(램)만을 사용하여 특유의 누린내 없이 부드러운 육질과 촉촉한 육즙을 자랑하는 전문점입니다.\n\n'
        '대표 메뉴인 고소한 양꼬치와 부드러운 양갈비를 비롯해 겉바속촉의 정석인 꿔바로우, 지삼선, 마라탕 등 다채로운 중화요리를 푸짐하게 맛볼 수 있습니다. '
        '향신료 \'쯔란\' 소스와 최고의 궁합을 자랑하며, 인심 후한 사장님의 친절한 서비스 덕분에 만족도가 매우 높습니다. 뚝섬 한강공원 나들이나 한강버스 탑승 후 시원한 칭다오 맥주를 곁들이며 하루를 마무리하기에 최적의 공간입니다.',
    dockName: '뚝섬',
    accessInfo: '서울특별시 광진구 동일로18길 69 (자양역·뚝섬한강공원 인근)',
    openingHours: '11:30 - 06:00 (연중무휴)',
    priceRange: '18,000원 - 67,000원',
  ),

  // ========== 압구정 ==========
  StoryItem(
    category: 'HISTORY',
    title: '한명회와 압구정 터',
    subtitle: '조선 최고 권력가의 야망이 머물던 정자',
    imageUrl: 'assets/images/apgujeong_site.jpg',
    description:
        '\'압구정(狎鷗亭)\'이라는 지명은 조선 전기 최고의 권력자였던 한명회가 한강 변에 세운 동명의 정자에서 유래했습니다. \'갈매기와 친하게 지낸다\'는 뜻으로 권력을 내려놓고 자연으로 돌아가겠다는 의미를 담았으나, 실제로는 수많은 정객들이 모이던 권력의 중심지였습니다.\n\n'
        '현재 정자는 유실되었고 압구정 현대아파트 단지 내에 그 터를 알리는 표석만이 남아 화려했던 과거를 증언합니다.',
    dockName: '압구정',
    accessInfo: '선착장에서 도보 10분',
    historicalPeriod: '조선 전기 (15세기)',
  ),
  StoryItem(
    category: 'HISTORY',
    title: '로데오거리와 청담동 패션',
    subtitle: '대한민국 트렌드와 오렌지족의 고향',
    imageUrl: 'assets/images/apgujeong_site.webp',
    description:
        '1990년대 대중문화와 트렌드의 중심지였던 압구정 로데오거리는 대한민국 유행을 선도하는 패션과 문화의 메카였습니다.\n\n'
        '수입 명품 브랜드와 디자이너 숍들이 밀집하며 부유층 청년 문화를 대변하는 공간이었고, 현재는 힙한 도산공원 상권과 연결되어 K-컬처와 다이닝 문화를 이끄는 핫플레이스로 진화했습니다.',
    dockName: '압구정',
    accessInfo: '선착장에서 도보 20분',
    historicalPeriod: '1990년대~현재',
  ),
  StoryItem(
    category: 'FOOD',
    title: '미미면가',
    subtitle: '미쉐린 가이드가 인정한 독창적인 토핑의 소바 전문점',
    imageUrl: 'assets/images/mimi_myeonga.jpeg',
    description:
        '압구정 선착장 인근 가로수길 상권에 위치한 미미면가는 미쉐린 가이드 빕 구르망에 꾸준히 이름을 올려온 서울의 대표적인 메밀소바 명소입니다. '
        '소바로 유명한 미슐랭 맛집으로, 고등어구이 온소바와 마즙과 성게알 온소바 등 독특한 메뉴가 인기입니다.\n\n',
    dockName: '압구정',
    accessInfo: '서울특별시 강남구 강남대로160길 29',
    openingHours: '11:30 - 21:30 (브레이크타임 15:00 - 17:30 / 라스트오더 20:30)',
    priceRange: '10,000원 - 23,000원',
  ),
  StoryItem(
    category: 'FOOD',
    title: '하하',
    subtitle: '압구정 상권의 지갑을 지켜주는 든든한 착한가격업소 일식당',
    imageUrl: 'assets/images/haha_japanese.jpeg',
    description:
        '신사동 골목에 위치한 \'하하\'는 화려하고 높은 물가를 자랑하는 압구정·가로수길 상권에서 보기 드문 뛰어난 가성비의 일식 분식 전문점입니다. '
        '치솟는 물가 속에서도 대중적이고 저렴한 가격에 양질의 식사를 제공하여 행정안전부와 강남구가 공식 지정한 \'착한 가격 업소\'로 당당히 선정된 찐 로컬 맛집입니다.\n\n',
    dockName: '압구정',
    accessInfo: '서울특별시 강남구 논현로151길 24',
    openingHours: '08:00 - 21:00 (일요일 정기휴무)',
    priceRange: '6,500원 - 12,000원',
  ),

  // ========== 잠실 ==========
  StoryItem(
    category: 'HISTORY',
    title: '뽕나무밭과 잠실나루',
    subtitle: '국가 양잠 산업의 중심지에서 초고층 도시로',
    imageUrl: 'assets/images/jamsil_history.jpeg',
    description:
        '\'잠실(蠶室)\'이라는 이름은 조선 전기 누에를 치고 명주실을 뽑던 국가 기관인 \'동잠실\'이 이곳에 설치된 것에서 유래했습니다. 당시 이 일대는 온통 뽕나무밭이었습니다.\n\n'
        '1970년대 한강 매립과 강남 개발을 통해 거대한 아파트 단지와 올림픽 주경기장이 들어서며 세계적인 스포츠·상업 도시로 대전환을 이루었습니다.',
    dockName: '잠실',
    accessInfo: '선착장에서 도보 10분',
    historicalPeriod: '조선시대 ~ 1970년대 개발',
  ),
  StoryItem(
    category: 'HISTORY',
    title: '삼전도비와 병자호란의 아픔',
    subtitle: '석촌호수 옆에 새겨진 뼈아픈 역사의 기록',
    imageUrl: 'assets/images/samjeondobi.jpeg',
    description:
        '잠실 선착장 인근 석촌호수 변에는 병자호란 당시 인조가 청나라 태종에게 무릎을 꿇고 항복했던 부끄러운 역사의 기록인 \'삼전도비(국가 지정 인문문화재)\'가 세워져 있습니다.\n\n'
        '화려한 롯데월드타워와 석촌호수의 아름다운 야경 뒤편에 숨겨진 역사의 이면을 돌아보며 평화의 소중함을 되새길 수 있는 장소입니다.',
    dockName: '잠실',
    accessInfo: '선착장에서 도보 20분',
    historicalPeriod: '조선 인조 (1639년 건립)',
  ),
  StoryItem(
    category: 'FOOD',
    title: '미엔아이',
    subtitle: '송리단길에서 만나는 대만식 우육미엔 전문점',
    imageUrl: 'assets/images/mienai.jpeg',
    description:
        '잠실 석촌호수 뒤편 송리단길의 터줏대감으로, 진하게 우려낸 한우 사골 육수에 야들야들한 아롱사태 고기를 올린 대만식 우육면을 선보입니다.\n\n'
        '수제 꿔바로우인 \'미엔아이 꿔바\'와 부드러운 마라 우육면의 조합이 일품이며, 이국적인 인테리어 덕분에 대만 야시장에 온 듯한 기분을 낼 수 있습니다.',
    dockName: '잠실',
    accessInfo: '서울특별시 송파구 오금로16길 5',
    openingHours: '11:20 - 21:00 (브레이크타임 14:30 - 17:20)',
    priceRange: '10,000원 - 15,000원',
  ),
  StoryItem(
    category: 'FOOD',
    title: '해주냉면',
    subtitle: '서울 3대 매운 냉면, 중독성 강한 불맛의 정수',
    imageUrl: 'assets/images/haejoo.jpeg',
    description:
        '1983년 잠실동에서 시작해 전국적인 매운맛 열풍을 이끌어온 명실상부한 노포 냉면집입니다. 알싸하고 강렬한 마늘 향과 고춧가루 베이스의 마법 양념이 특징입니다.\n\n'
        '뇌를 자극하는 화끈한 매운 비빔냉면을 먹다가 셀프로 제공되는 뜨겁고 짭조름한 육수로 입안을 달래주는 것이 이곳을 즐기는 정석입니다.',
    dockName: '잠실',
    accessInfo: '서울특별시 송파구 올림픽로12길 5-5',
    openingHours: '11:30 - 21:00 (일요일 휴무)',
    priceRange: '8,000원 - 9,000원',
  ),
];

final List<FaqItem> faqDataKo = [
  FaqItem(
    question: '기후동행카드를 사용할 수 있나요?',
    answer: '네, 가능합니다. 단, 따릉이 결합권이 아닌 일반권/청년권 등을 확인해주세요.',
  ),
  FaqItem(question: '반려동물 탑승이 가능한가요?', answer: '케이지(이동장)에 넣은 경우에만 탑승이 가능합니다.'),
  FaqItem(
    question: '유모차나 자전거를 실을 수 있나요?',
    answer: '유모차는 가능하며, 자전거는 선착장 내 거치대에 보관하셔야 합니다 (선내 반입 불가).',
  ),
];
